...
plotHeatmapExpr <-
  function(genes,
    ...
    textSize = 10,
    plotTitle = "Heatmap"
    # heatmapCol = circlize::colorRamp2(c(minExpr, maxExpr), c("yellow3" , "navy"))
    ...
    if (orgTum == "Org") {
      annotData <-
        # annotData %>% arrange(Response_Treatment, PatientID, Tissue)
      annotData %>% arrange(Treatment_Response, PatientID, Tissue)
      ddgene <- ddgene[, annotData$UniqueIDs_FOLFOX]
